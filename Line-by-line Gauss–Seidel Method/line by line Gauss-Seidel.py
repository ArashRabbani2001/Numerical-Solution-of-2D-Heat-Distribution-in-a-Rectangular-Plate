...
for iteration in range(1, max_iter+1):
    ...
    for i in range(1, Ny - 1):
        for j in range(1, Nx - 1):
            T_old = T[i, j]
            T_new = 0
            T[i,j] = (T[i+1,j]+T[i-1,j]+((beta**2*T[i,j+1])+(beta**2*T[i,j-1])))/(2*(1+beta**2))
            T_new = T_old + relaxation_factor*(T[i,j]-T_old)
            old_index_collector.append(T_old)
            new_index_collector.append(T_new)
    ...
    if tolerance_flag == True:
        ...
        with open("line-by-line results.txt" , "w") as file:
            file.write("Result:" + "\n")
            for row in T:
        ...
        ax.set_title("dimension of 30cm*40cm - 1cm intervals - line by line Gauss-Seidel")
        ...
        plt.title('dimension of 30cm*40cm - 1cm intervals - line by line Gauss-Seidel')
